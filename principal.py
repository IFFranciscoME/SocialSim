...
param_a_v_p = [t, dat.n_canales_a, dat.param_beta_a, dat.p_total_A, dat.k_plantas_porcentaje, # Parametros de f_serie_tiempo_visitan
                           m_bin_comb, v_prob_comb, v_prob_cant, dat.v_plantas_p, dat.v_plantas_c, dat.v_plantas_h]
param_b_v_p = [t, dat.n_canales_b, dat.param_beta_b, dat.p_total_B, dat.k_plantas_porcentaje, # Parametros de f_serie_tiempo_visitan
                           m_bin_comb, v_prob_comb, v_prob_cant, dat.v_plantas_p, dat.v_plantas_c, dat.v_plantas_h]
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
param_a_v_c = [t, dat.n_canales_a, dat.param_beta_a, dat.p_total_A, dat.k_comidas_porcentaje, # Parametros de f_serie_tiempo_visitan
                           m_bin_comb_c, v_prob_comb_c, v_prob_cant, dat.v_comidas_p, dat.v_comidas_c, dat.v_comidas_h]
param_b_v_c = [t, dat.n_canales_b, dat.param_beta_b, dat.p_total_B, dat.k_comidas_porcentaje, # Parametros de f_serie_tiempo_visitan
                           m_bin_comb_c, v_prob_comb_c, v_prob_cant, dat.v_comidas_p, dat.v_comidas_c, dat.v_comidas_h]
...
param_a_c = [param_v_sector_A_prob_acom, dat.min_acomp_A, dat.porcentaje_baño, 
                                         dat.baño_insumo_c, dat.porcentaje_taller_A, dat.taller_insumo_c, dat.costo_total_fijo/2]
param_b_c = [param_v_sector_B_prob_acom, dat.min_acomp_B, dat.porcentaje_baño, 
                                         dat.baño_insumo_c, dat.porcentaje_taller_B, dat.taller_insumo_c, dat.costo_total_fijo/2]
...
keys_u = ['Utilidad_A-Plantas', 'Utilidad_A-Comidas','Utilidad_B-Plantas','Utilidad_B-Comidas']

utilidad_segmento = [pd.concat([Df_A_plantas_flujo[i].iloc[:,3], Df_A_comidas_flujo[i].iloc[:,3], 
        Df_B_plantas_flujo[i].iloc[:,3],Df_B_comidas_flujo[i].iloc[:,3] ], axis = 1, keys = keys_u) for i in range(n)]

utilidad_total = pd.concat([pd.concat([Df_A_plantas_flujo[i].iloc[:,3], Df_A_comidas_flujo[i].iloc[:,3], 
        Df_B_plantas_flujo[i].iloc[:,3],Df_B_comidas_flujo[i].iloc[:,3] ], axis = 1).T.sum() for i in range(n)], axis = 1)
...
horas_segmento = [pd.concat([Df_A_plantas_completo[i].iloc[:,10], Df_A_comidas_completo[i].iloc[:,10], 
        Df_B_plantas_completo[i].iloc[:,10],Df_B_comidas_completo[i].iloc[:,10] ], axis = 1, keys = keys_h) for i in range(n)]


keys_sim = list(np.arange(n).astype('U'))

horas_total = pd.concat([pd.concat([Df_A_plantas_completo[i].iloc[:,10], Df_A_comidas_completo[i].iloc[:,10], 
        Df_B_plantas_completo[i].iloc[:,10],Df_B_comidas_completo[i].iloc[:,10] ], axis = 1).T.sum() for i in range(n)], axis = 1, keys = keys_sim)
...
ven_mural = pd.DataFrame(sim.f_familias_mural(asamblea_total, dat.total_familias, 
                                 dat.porcentaje_familias_van_casa, dat.porcentaje_familias_ven_mural), index = keys_sim).T
