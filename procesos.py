...
def f_visitas_segmento(n_canales, param_beta, param_segmento, param_t_visitan):
    ...
    personas_visitan_total = [int(personas_visitan[i] + personas_regresan[i]) for i in
                              range(n_canales)]
...
def f_serie_tiempo_visitan(param_n_periodos, n_canales, param_beta, param_segmento):
    ...
    for i in range(param_n_periodos):
        vi, ci, ri, pci = f_visitas_segmento(n_canales, param_beta, param_segmento, v[i])
        v.append(vi)
        c.append(ci)
        r.append(ri)
        pc.append(pci)
...
def f_n_simulaciones_proceso(n_sim, list_parameters_ventas, list_parameters_costos):
    ...
    obj = [f_ventas_total(list_parameters_ventas[0], list_parameters_ventas[1],
                          ...
                          list_parameters_ventas[9], list_parameters_ventas[10]) for i in
           range(n_sim)]
    # Por canal
...
def f_ts_costos(param_v_sector_prob_acomp, param_v_visitantes, param_k_min_acomp,
    ...
    Parameters
    ----------
    param_v_sector_prob_acomp : list : vector con las probabilidades de acompanantes
    param_v_visitantes : list : vector de visitantes en el periodo
    param_k_min_acomp : int : numero acompanantes
    param_porcentaje_bano : float : porcentaje de los visitantes que van al bano
    param_costo_bano : int : costo de mantenimiento de bano
